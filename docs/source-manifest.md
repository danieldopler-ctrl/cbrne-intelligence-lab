...
| PHMSA Hazmat Incident Reports - Data Mining Tool | Real export imported and detection run validated | Export uses `Date Of Incident`, `Commodity Long Name`, `Total Hazmat Fatalities`, `Hazmat Injury Indicator`, and `Serious Evacuations`; Yes/No indicators are conservatively mapped to presence/absence of consequences; raw quantity/UOM remain unscored pending conversion rules | Consequence-rule source for fatalities and indicated injury/serious evacuation records | Indicator fields do not provide counts; transportation incident reporting does not establish intent; one incident may appear in multiple rows |
...
| 2026-05-24 | PHMSA official export / batch 3 | 1,777 | `CHEM_HAZMAT_V0.2`: 269 consequence alerts, 789 EPA RMP toxic-substance commodity matches, and 6 bounded recurrence indicators | PHMSA quantity/UOM remains unscored; repeated incident rows require deduplication before aggregate reporting |
